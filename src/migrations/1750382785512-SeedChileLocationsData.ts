...
export class SeedChileLocationsData1750382785512 implements MigrationInterface {
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		for (const region of chileData.regions) {
			...
			for (const province of region.provincias) {
				...
				await queryRunner.query(
					`INSERT INTO provinces (id, name, countryId, regionId) VALUES ('${provinceId}', '${escapeSingleQuotes(province.name)}', '${countryId}', '${regionId}')`,
				);

				// Insertar comunas
				for (const district of province.comunas) {
